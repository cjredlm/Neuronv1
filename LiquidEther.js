/**
 * LiquidEther - Vanilla JS adaptation of the React component.
...
class LiquidEther {
  constructor(options = {}) {
    ...
    this.webgl = null;
    this.isVisible = true;
    console.log('LiquidEther: Initializing with options', this.options);
    this.init();
  }

  init() {
    this.makePaletteTexture = (stops) => {
      let arr;
    ...
    const paletteTex = this.makePaletteTexture(this.options.colors);
    ...
    class CommonClass {
      ...
      init(container) {
        ...
        // Limitar pixel ratio a 1 y desactivar antialias en móviles para salvar FPS
    ...
    const color_frag = `
      ...
      void main(){
        ...
        // Base fluid color
        vec3 fluidC = texture2D(palette, vec2(lenv, 0.5)).rgb;
        
        vec3 finalRGB = mix(bgColor.rgb, fluidC, lenv);
        float outA = mix(bgColor.a, 1.0, lenv);
        gl_FragColor = vec4(finalRGB, outA);
      }
    `;
    ...
    class WebGLManager {
      ...
      init() {
        console.log('LiquidEther: WebGLManager init starting');
        this.props.$wrapper.prepend(Common.renderer.domElement);
        this.output = new Output();
        console.log('LiquidEther: WebGLManager init complete');
      }
      resize() {
        Common.resize();
        this.output.resize();
    ...
    // Positioning and overflow are handled via CSS classes
    ...
    const applyOptions = () => {
      ...
      if (this.options.resolution !== prevRes) {
        sim.resize();
      }
    };
    applyOptions();
...
// Attach to window so it's accessible in main.js
